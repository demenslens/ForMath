#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""relatie_manager.py — PoC voor het relatie-mechanisme (zandbak, standalone).
...
Dit bestand is de ENIGE gezaghebbende definitie van de canonieke
prefix-serialisatie (één gezaghebbende bron). De JS-kant
(reductie_helpers.js) spiegelt haar en wordt via een gedeelde test-vector
(de vingerafdruk in data/relaties.json) tegen deze definitie geverifieerd
door harness.js.

CLI:
    python3 relatie_manager.py data/            # vul vingerafdrukken in,
                                                # valideer, genereer ui/data.js
    python3 relatie_manager.py data/ --check    # alleen valideren (dry-run)
...
def genereer_ui_data(data_dir, opgaven_by_id, registry):
    """Schrijf ui/data.js zodat de mini-UI zónder server (file://) kan laden.
    GEGENEREERD bestand — de bron blijft data/*.json (één gezaghebbende bron)."""
    ui_pad = Path(data_dir).parent / "ui" / "data.js"
    ui_pad.parent.mkdir(exist_ok=True)
    blob = json.dumps({"opgaven": opgaven_by_id, "relaties": registry},
...
def main(argv=None):
    ...
    if not args.check:
        ui_pad = genereer_ui_data(data_dir, opgaven, registry)
        print("\n→ %s gegenereerd (voor de mini-UI onder file://)" % ui_pad)
