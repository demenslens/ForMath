...
- **Leg elke werksessie vast.** Is er iets wezenlijks uitgezocht of veranderd,
  schrijf dan aan het eind één document
  `WERKSESSIE_<jjjj-mm-dd>_<onderwerp>.md`, in de map van het onderdeel waar het
  over ging, en zet er een regel bij in [`WERKSESSIES.md`](WERKSESSIES.md) (in
  deze map).

  Wat erin hoort: de opdracht, wát er gemeten is en met welke uitkomst, wat er is
  veranderd en waaróm, wat openblijft, en de commits. Wat er niet in hoort: alles
  wat de code zelf al zegt.

  Waarom: het dure deel van een sessie zit in wat er gemeten en verwórpen is — de
  aanname die niet bleek te kloppen, het cijfer waar een constante uit volgt, de
  reden dat iets zó en niet anders is opgelost. Dat overleeft geen
  commit-boodschap en is niet uit de code terug te lezen, terwijl de volgende
  sessie er anders weer op stukloopt.
