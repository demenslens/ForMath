...
def _render_expression(node, computed_ids, _top_level=True):
    ...
    if t == 'SIMPLIFY_OP':
        source = node.get('source', {})
        inner = _render_expression(source, computed_ids, _top_level=False)
        if neg:
            return f"-({inner})"
    ...
    if t == 'UNARY_OP':
        op = node.get('operator', '-')
        operand = node.get('operand', {})
        operand_str = _render_expression(operand, computed_ids, _top_level=False)
...
def _node_to_mathjson(node, path, pyid_to_bid, pyid_to_info, node_map):
    ...
    if t == 'SIMPLIFY_OP':
        ...
        if source is None:
            ...
            source_mj = _node_to_mathjson(source, path + [0], pyid_to_bid,
