...
def generate_formath_json(converted_ast, latex, mathml='',
    ...
    # Export-validatie: draai de vijf checks (dekking, paden, Negate,
    # dubbele negatie, waarde-invariantie) op de zojuist gebouwde opgave.
    # Een probleem hier betekent dat de export niet klopt; we melden het
    # luid zodat het bij het maken opvalt in plaats van pas in de studenttool.
    try:
        try:
            from export_validatie import valideer_export
        except ImportError:
            from .export_validatie import valideer_export
        _export_problemen = valideer_export(ast_tree, ast_node_map, mathblocks, duo)
        if _export_problemen:
            print(f"⚠️  EXPORT-CHECK: {len(_export_problemen)} probleem(en) in opgave {opgave_id}:")
            for _p in _export_problemen:
                print("    " + _p)
        else:
            print(f"✓ EXPORT-CHECK: opgave {opgave_id} schoon (5/5 checks).")
...
def _traverse(node, result, order_counter=None):
    ...
    t = node.get('type')
    is_op = t in ('BINARY_OP', 'MANIFOLD_OP', 'POWER', 'ROOT',
                  'MATROESJKA_OP', 'SIMPLIFY_OP', 'MIXED_NUMBER_OP',
                  'UNARY_OP')
    ...
    for n in sorted_ops:
        ...
        if t_node == 'SIMPLIFY_OP':
            ...
            if mt == 0:
                ...
                # Negatief gemengd getal: -(g+t/n), niet -g+t/n
...
def _render_expression(node, computed_ids, simplify_source_ids=None,
                       _top_level=True):
    """
    Render de AST als leesbare expressie string.
    Nodes waarvan id(node) in computed_ids zit, worden vervangen door hun uitkomst.

    simplify_source_ids: set van id(node) voor nodes die source zijn van een
        SIMPLIFY_OP. Wanneer zo'n source berekend is (in computed_ids), tonen
        we de RUWE uitkomst (60/72) via evaluate_raw i.p.v. de automatisch
        vereenvoudigde Fraction (5/6). Hierdoor wordt de vereenvoudig-stap
        zichtbaar als aparte transformatie in de duo_verzameling.
    ...
    if py_id in computed_ids:
        # SIMPLIFY_OP zelf berekend: toon vereenvoudigde vorm uit veld.
        # Het teken van de source zit al in vereenvoudigd.teller (via
        # evaluate_raw in de injector). De wrapper zelf heeft in de praktijk
        # geen is_negative=True, maar voor robustheid passen we hier nog wel
        # de neg-wrap toe op de wrapper-flag.
        if t == 'SIMPLIFY_OP':
            verv = node.get('vereenvoudigd', {})
            vt, vn = verv.get('teller'), verv.get('noemer')
            if vt is None or vn is None:
                ...
                # evaluate() heeft is_negative al meegenomen; geen neg-wrap.
                return result_str
            elif vn == 1:
                result_str = str(vt)
            else:
                result_str = f"{vt}/{vn}"
            return f"-({result_str})" if neg else result_str

        # MIXED_NUMBER_OP zelf berekend: idem qua redenering als SIMPLIFY_OP.
        if t == 'MIXED_NUMBER_OP':
            ...
            if mt == 0:
                ...
                result_str = f"-({abs(geheel)}+{abs(mt)}/{mn})"
            else:
                result_str = f"{geheel}+{abs(mt)}/{mn}"
            return f"-({result_str})" if neg else result_str

        # Source van een SIMPLIFY_OP: toon RUWE uitkomst (vóór vereenvoudiging)
        # via evaluate_raw. LET OP: evaluate_raw neemt is_negative van de node
        # al mee in de (teller, noemer) — dus GEEN extra neg-wrap toepassen.
        if py_id in simplify_source_ids:
            try:
                from ast_visualizer import evaluate_raw
            except ImportError:
                from .ast_visualizer import evaluate_raw
            ...
            if raw is not None:
                ...
                return f"{rt}/{rn}"
            val = evaluate(node)
            return format_result(val) if val is not None else '?'

        # Normaal geval: evaluate() geeft de Fraction terug INCLUSIEF
        # is_negative. Geen extra neg-wrap toepassen.
        val = evaluate(node)
        return format_result(val) if val is not None else '?'
    ...
    if t == 'POWER':
        ...
        base_str = _render_expression(base_node, computed_ids, simplify_source_ids, _top_level=False)
        ...
        if base_t == 'POWER' and id(base_node) not in computed_ids:
            base_str = f"({base_str})"
        # Haakjes rond een NEGATIEVE base: -40^2 leest als -(40^2), maar
        # een berekende/negatieve base bedoelt (-40)^2. Zonder haakjes klapt
        # de waarde om. Geldt zowel voor een leaf-minteken als voor een
        # berekende base die negatief uitkomt (begint met '-').
    ...
    if t == 'ROOT':
        ...
        radicand_str = _render_expression(radicand_node, computed_ids, simplify_source_ids, _top_level=False)
        idx = node.get('index', {})
        idx_val = idx.get('value', '?') if idx.get('type') == 'NUMBER' else '?'
        if str(idx_val) == '2':
    ...
    if t == 'BINARY_OP':
        op = node.get('operator', '+')
        left_str = _render_expression(node.get('left', {}), computed_ids, simplify_source_ids, _top_level=False)
        right_node = node.get('right', {})
        right_str = _render_expression(right_node, computed_ids, simplify_source_ids, _top_level=False)
    ...
    if t == 'MANIFOLD_OP':
        ...
        for operand in node.get('operands', []):
            part = _render_expression(operand, computed_ids, simplify_source_ids, _top_level=False)
            parts.append(part)
    ...
    # SIMPLIFY_OP — vereenvoudig-wrapper. Zelf geen wiskundig effect op de
    # AST-structuur; render gewoon de source. Als de simplify-stap zelf al
    # is uitgevoerd (in computed_ids) wordt de afhandeling op regel 905
    # bovenaan deze functie afgevangen.
    if t == 'SIMPLIFY_OP':
        source = node.get('source', {})
        inner = _render_expression(source, computed_ids, simplify_source_ids, _top_level=False)
        if neg:
            return f"-({inner})"
        # _top_level overerven van bovenste aanroep: SIMPLIFY_OP is doorzichtig,
        # dus geen extra haakjes
        return inner

    # MIXED_NUMBER_OP — idem, een doorzichtige wrapper rond source.
    if t == 'MIXED_NUMBER_OP':
        source = node.get('source', {})
        inner = _render_expression(source, computed_ids, simplify_source_ids, _top_level=False)
        if neg:
    ...
    # UNARY_OP — bv. unaire min op een sub-expressie: -(a+b)
    if t == 'UNARY_OP':
        op = node.get('operator', '-')
        operand = node.get('operand', {})
        operand_str = _render_expression(operand, computed_ids, simplify_source_ids, _top_level=False)
        # Haakjes als operand een operatie is (visueel duidelijker)
        operand_t = operand.get('type')
        needs_brackets = operand_t in ('BINARY_OP', 'MANIFOLD_OP',
                                       'MATROESJKA_OP', 'POWER', 'ROOT')
        if needs_brackets and not operand_str.startswith('('):
            operand_str = f"({operand_str})"
        inner = f"{op}{operand_str}"
        if neg:
            return f"-({inner})"
        return inner

    # MATROESJKA_OP — niet (volledig) ondersteund door deze renderer.
    # Het type wordt door manifold_converter aangemaakt voor links-associatieve
    # ketens van : en ×. Een correcte rendering vereist het uitvouwen van
    # de shells-structuur naar een keten als a:b:c. Voor nu: diagnose-print
    # zodat het zichtbaar wordt wanneer dit voorkomt, plus een best-effort
    # fallback die niet '?' is.
    if t == 'MATROESJKA_OP':
        print(f"[BUG][_render_expression] MATROESJKA_OP nog niet volledig "
              f"ondersteund; shells={len(node.get('shells', []))}")
    ...
    # Fallback voor echt onbekende types — diagnose-print zodat we het
    # weten in plaats van stille '?'.
    print(f"[BUG][_render_expression] Onbekend node-type: {t!r}  "
...
def _build_duo(op_nodes, node_lookup, converted_ast=None):
    ...
    Per step N:
    - input_expressie: de expressie aan het begin van deze step.
      Alleen hoog-blocks van vorige steps zijn berekend; deze input is
      voor ALLE mathblocks (hoog en laag) in deze step hetzelfde.
    - hoog: lijst van {mathblock, output_expressie}.
      Per hoog-block de output-expressie die ontstaat als ALLEEN dat ene
      hoog-block wordt uitgevoerd vertrekkend vanaf input_expressie.
    - laag: lijst van {mathblock, output_expressie}.
      Mathblocks uit hogere steps die direct kunnen omdat al hun
      operatie-inputs bij step < N berekend zijn (en die zelf niet
      afhankelijk zijn van andere laag-blocks). Per laag-block de
      output-expressie alsof alleen dat ene laag-block extra wordt
      uitgevoerd bovenop input_expressie.

    Veldvolgorde per entry: step, input_expressie, hoog, laag.

    Als converted_ast is None: hoog en laag bevatten alleen block-IDs
    (geen output_expressie), en input_expressie ontbreekt.
    """
    by_step = defaultdict(list)
    for n in op_nodes:
        by_step[n['step']].append(n)
    ...
    # Running set: hoog-blocks van vorige steps die "officieel" zijn berekend.
    # Laag-blocks worden NIET meegenomen naar de volgende step.
    hoog_computed = set()

    for step in range(1, max_step + 1):
        hoog_bids = [n['block_id']
                     for n in sorted(by_step.get(step, []),
                                     key=lambda x: x['order'])]

        # ── Kandidaat-laag-blocks: inputs uit strikt eerdere steps ──
        laag_candidates = []
        for n in op_nodes:
            if n['step'] <= step:
                continue
        ...
        # Filter: geen laag-block dat van een andere laag-kandidaat afhangt
        laag_pyids = {n['py_id'] for n in laag_candidates}
        laag_bids = []
        for n in laag_candidates:
            depends_on_laag = any(c in laag_pyids
                                  for c in n['children_py_ids'])
        ...
        if converted_ast is not None:
            # Input-expressie: gelijk voor alle blocks in deze step
            input_expr = _render_expression(converted_ast, hoog_computed,
                                            simplify_source_ids)
            entry['input_expressie'] = input_expr

            # Hoog: per block de output als ALLEEN dat block uitgevoerd wordt
            hoog_list = []
            for bid in hoog_bids:
                pyid = bid_to_pyid.get(bid)
                if pyid:
                    temp = hoog_computed | {pyid}
                    out_expr = _render_expression(converted_ast, temp,
                                                  simplify_source_ids)
                else:
                    out_expr = '?'
                hoog_list.append(OrderedDict([
                    ('mathblock', bid),
                    ('output_expressie', out_expr),
                ]))
            entry['hoog'] = hoog_list

            # Laag: idem, alleen dat ene laag-block extra bovenop input
            laag_list = []
            for bid in laag_bids:
                ...
                if pyid:
                    ...
                    out_expr = '?'
                laag_list.append(OrderedDict([
                    ('mathblock', bid),
                    ('output_expressie', out_expr),
                ]))
            entry['laag'] = laag_list

            # Running set bijwerken: hoog-blocks van DEZE step zijn nu
            # officieel berekend voor de volgende step
            for bid in hoog_bids:
                pyid = bid_to_pyid.get(bid)
                if pyid:
                    hoog_computed.add(pyid)
        else:
            # Geen AST beschikbaar: alleen block-ID-lijstjes
            entry['hoog'] = hoog_bids
            entry['laag'] = laag_bids
...
def _build_mathjson_ast(converted_ast, op_nodes, node_lookup):
    ...
    # Verifieer dat de node_map klopt met de tree en met de mathblocks-lijst.
    # Print waarschuwingen bij afwijkingen; faalt niet hard (eerst observeren).
    expected_bids = {n['block_id'] for n in op_nodes if n.get('block_id')}
    _validate_node_map(tree, node_map, expected_bids)

    return tree, node_map


def _validate_node_map(tree, node_map, expected_block_ids):
    """
    Valideer de node_map tegen de tree en de verwachte mathblock-IDs.

    Drie checks (waarschuwingen, geen exceptions):
    A. Elk mathblock_id uit expected_block_ids heeft een 'operation'-entry
       in node_map.
    B. Elk pad in node_map verwijst naar een bestaand knooppunt in tree.
    C. Voor 'operation'-entries: het knooppunt op het pad is GEEN
       ["Negate", ...]-wrapper. (Anders is Fix 1 niet correct toegepast
       en wijst het pad nog steeds naar een Negate ipv naar de bewerking.)
    """
    warnings = []

    # Check A — mathblock-dekking
    op_entry_bids = {e.get('mathblock_id') for e in node_map
                     if e.get('type') == 'operation' and e.get('mathblock_id')}
    missing = expected_block_ids - op_entry_bids
    if missing:
        warnings.append(
            f"[NODE_MAP] Mathblocks zonder operation-entry: "
            f"{sorted(missing)}"
        )

    # Check B + C — pad-validiteit en Negate-uitlijning
    for entry in node_map:
        path = entry.get('path', [])
        bid = entry.get('mathblock_id', '?')
        etype = entry.get('type', '?')

        # Navigeer langs het pad in de tree
        node = tree
        path_valid = True
        for idx, step in enumerate(path):
            # tree[0] is operatornaam, tree[1..] zijn argumenten.
            # path-indices zijn argument-indices (Eis A uit de spec),
            # dus tree[step + 1].
            if not isinstance(node, list):
                warnings.append(
                    f"[NODE_MAP] Pad {path} voor {bid}: stap {idx} ({step}) "
                    f"maar huidige knoop is geen array: {node!r}"
                )
                path_valid = False
                break
            arg_index = step + 1
            if arg_index < 1 or arg_index >= len(node):
                warnings.append(
                    f"[NODE_MAP] Pad {path} voor {bid}: stap {idx} ({step}) "
                    f"buiten argumenten-bereik van {node[0]!r} "
                    f"(heeft {len(node) - 1} arg(s))"
                )
                path_valid = False
                break
            node = node[arg_index]
        ...
        # Check C — operation-entry mag niet op een Negate-wrapper wijzen
        if etype == 'operation':
            if isinstance(node, list) and len(node) >= 1 and node[0] == 'Negate':
                warnings.append(
                    f"[NODE_MAP] Pad {path} voor {bid} wijst naar een "
                    f"Negate-wrapper i.p.v. naar de bewerking zelf. "
                    f"Fix 1 is hier niet correct toegepast."
                )
...
def _node_to_mathjson(node, path, pyid_to_bid, pyid_to_info, node_map):
    ...
    if t == 'BINARY_OP':
        ...
        # Bij is_negative wordt de operatie ingepakt in ["Negate", inner].
        # De operatie-array zit dan in de tree op path + [0], niet op path.
        # Recursieve aanroepen en node_map-registratie moeten dit pad gebruiken
        # zodat álle paden in de subtree correct blijven uitlijnen met de tree.
        inner_path = path + [0] if neg else path

        left_mj = _node_to_mathjson(left, inner_path + [0], pyid_to_bid, pyid_to_info, node_map) if left else 0
        right_mj = _node_to_mathjson(right, inner_path + [1], pyid_to_bid, pyid_to_info, node_map) if right else 0
        ...
        # Registreer operatie in node_map op het pad van de bewerking zelf,
        # NIET op het pad van een eventuele Negate-wrapper.
        if block_id:
            node_map.append(OrderedDict([
                ('path', list(inner_path)),
    ...
    if t == 'MANIFOLD_OP':
        ...
        for i, operand in enumerate(operands):
            child_mj = _node_to_mathjson(operand, inner_path + [i], pyid_to_bid, pyid_to_info, node_map)
    ...
    if t == 'POWER':
        ...
        base_mj = _node_to_mathjson(base, inner_path + [0], pyid_to_bid, pyid_to_info, node_map) if base else 0
    ...
    if t == 'ROOT':
        ...
        radicand_mj = _node_to_mathjson(radicand, inner_path + [0], pyid_to_bid, pyid_to_info, node_map) if radicand else 0
    ...
    # ── SIMPLIFY_OP ──
    # Vereenvoudig-mathblock. Sinds mei 2026 een eigen niveau in de tree:
    # ["Simplify", source_mj]. De source krijgt path + [0] (eerste argument-
    # positie). Dit is consistent met andere operatie-typen die ook een
    # eigen niveau hebben.
    if t == 'SIMPLIFY_OP':
        source = node.get('source')
        inner_path = path + [0] if neg else path
        if source is None:
            ...
            source_mj = _node_to_mathjson(source, inner_path + [0], pyid_to_bid,
                                          pyid_to_info, node_map)
        inner = ["Simplify", source_mj]
        if block_id:
            node_map.append(OrderedDict([
                ('path', list(inner_path)),
                ('mathblock_id', block_id),
    ...
    # ── MIXED_NUMBER_OP ──
    # Gemengd-getal-mathblock. Sinds mei 2026 een eigen niveau in de tree:
    # ["MixedNumber", source_mj]. Source op path + [0].
    if t == 'MIXED_NUMBER_OP':
        source = node.get('source')
        inner_path = path + [0] if neg else path
        if source is None:
    ...
    if t == 'UNARY_OP':
        ...
        if operand is None:
            return 0
        inner_path = path + [0] if neg else path
        operand_mj = _node_to_mathjson(operand, inner_path + [0], pyid_to_bid,
                                       pyid_to_info, node_map)
        if op == '-':
