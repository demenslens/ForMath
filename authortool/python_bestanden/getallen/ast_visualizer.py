...
def draw_nodes(svg, info, dx, dy, block_ids=None, simplify_source_ids=None,
    ...
    # node_paths een entry heeft voor deze node, tonen we het pad. Dit
    # is consistent met de node_map in de JSON die ook leaves bevat.
    if id(node) in node_paths:
        p = list(node_paths[id(node)])
        path_str = "path = " + repr(p)
        ET.SubElement(svg, "text", {
            "x": str(round(x - w / 2 + 5, 1)),
            "y": str(round(y + h + 12, 1)),
            "text-anchor": "start",
            "font-family": "JetBrains Mono, Consolas, monospace",
            "font-size": "9",
            "font-weight": "400",
...
def _compute_paths_for_blocks(ast):
    """
    Bereken voor elke node (operatie EN leaf) in de AST zijn pad als list
    van indices.

    De pad-indexering MOET overeenkomen met _node_to_mathjson in
    json_exporter.py, zodat de paden in de SVG (visualisatie) exact
    dezelfde zijn als die in metadata.expressie.ast.node_map (JSON).

    Returns:
        dict: id(node) → tuple van indices
              Voor zowel operatie-nodes als leaves (NUMBER/FRACTION/PARAMETER).

    Indexering per type:
    - BINARY_OP:       left → path+[0],   right → path+[1]
    - MANIFOLD_OP:     operands[i] → path+[i]
    - POWER:           base → path+[0]
    - ROOT:            radicand → path+[0]
    - UNARY_OP:        operand → path+[0]
    - SIMPLIFY_OP:     source → path+[0]    (sinds mei 2026 niet meer doorzichtig)
    - MIXED_NUMBER_OP: source → path+[0]    (sinds mei 2026 niet meer doorzichtig)
    - MATROESJKA_OP:   NOG NIET ONDERSTEUND in _node_to_mathjson. We
      gebruiken hier dezelfde traversal als assign_block_ids/collect_nodes,
      zodat de SVG wél een pad-label heeft. Indexering: shells[0].left
      → path+[0], shells[i].right → path+[i+1]. Dit blijft een lokale
      best-effort afspraak voor de SVG; de JSON heeft hier (nog) geen
      tegenhanger.

    Returns:
        dict: id(node) → tuple van indices  (alleen voor operatie-nodes)
    """
    paths = {}
    ...
    def walk(node, path):
        ...
        # Leaves: registreer ook hun pad zodat de SVG een 'path = [...]'
        # label kan tonen onder de externe-input-rect (consistent met
        # operatie-nodes en met de leaf-entries in node_map).
        ...
        # Operation-nodes: zelf op huidig pad, kinderen via index
        paths[id(node)] = tuple(path)
        ...
        if t == 'BINARY_OP':
            ...
            for i, op in enumerate(node.get('operands', [])):
                walk(op, path + [i])
        elif t == 'POWER':
            base = node.get('base')
            if base is not None:
            ...
            if radicand is not None:
                walk(radicand, path + [0])
        elif t == 'UNARY_OP':
            operand = node.get('operand')
            if operand is not None:
                walk(operand, path + [0])
        elif t == 'SIMPLIFY_OP':
            # Source op path + [0] (eerste argument na operator-naam)
            source = node.get('source')
            ...
            # Best-effort: shells[0].left op [0], shells[i].right op [i+1]
            shells = node.get('shells', [])
            for i, shell in enumerate(shells):
                if i == 0:
                    left = shell.get('left')
